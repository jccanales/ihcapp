...
(function(i) {
    ...
    var e = google.maps, o = new e.Geocoder, k = 0, p = 0, f = {}, f = {init: function(a) {
            ...
            return this.each(function() {
                ...
                if (0 !== c.controls.length)
                    for (b = 0; b < c.controls.length; b += 1)
                        g.controls[c.controls[b].pos].push(c.controls[b].div);
            ...
            if (a) {
                for (b = 0; b < a.length; b += 1)
                    a[b].latitude && a[b].longitude && (c > a[b].latitude && (c = a[b].latitude), h < a[b].longitude && (h = a[b].longitude), d > a[b].longitude && (d = a[b].longitude), g < a[b].latitude && (g = a[b].latitude), console.log(a[b].latitude, a[b].longitude, c, h, d, g));
            ...
            if (a) {
                for (b = 0; b < a.length; b += 1)
                    c > a[b].getPosition().lat() && (c = a[b].getPosition().lat()), h < a[b].getPosition().lng() && (h = a[b].getPosition().lng()), d > a[b].getPosition().lng() && (d = a[b].getPosition().lng()), g < a[b].getPosition().lat() && (g = a[b].getPosition().lat());
                f._boundaries = {N: c, E: h, W: d, S: g}
            }
            -1E3 == c && (f._boundaries = {N: 0, E: 0, W: 0, S: 0});
            return f._boundaries
            ...
            var c, h = this, d, g;
            if (a.markers.length && ("fit" == a.latitude || "fit" == a.longitude))
                return d = b ? f._getBoundariesFromMarkers.apply(this) : f._getBoundaries(a), c = new e.LatLng((d.N + d.S) / 2, (d.E + d.W) / 2), console.log(b, d, c), c;
            if (a.latitude && a.longitude)
                return c = new e.LatLng(a.latitude, a.longitude);
            c = new e.LatLng(0, 0);
            if (a.address)
                return o.geocode({address: a.address}, function(b, c) {
            ...
            if (0 < a.markers.length) {
                ...
                if (g.latitude && g.longitude)
                    return new e.LatLng(g.latitude, g.longitude);
                g.address && o.geocode({address: g.address}, function(b, c) {
                    c === google.maps.GeocoderStatus.OK ? f._setMapCenter.apply(h, [b[0].geometry.location]) : a.log && console.log("Geocode was not successful for the following reason: " + c)
                })
            }
            return c
            ...
            for (d = 0; d < c.length; d += 1)
                c[d].getMarker().setMap(null);
            c.length = 0;
            if (d = a.gmap.getBounds()) {
                var g = d.getNorthEast(), e = d.getSouthWest(), j = [], l = (g.lat() - e.lat()) * h.clustering.clusterSize / 100;
                for (d = 0; d < b.length; d += 1)
                    b[d].latitude < g.lat() && b[d].latitude > e.lat() && b[d].longitude < g.lng() && b[d].longitude > e.lng() && (j[j.length] = b[d]);
            ...
            if (!b)
                var i = {image: f.icon.image, iconSize: new e.Size(f.icon.iconsize[0], f.icon.iconsize[1]), iconAnchor: new e.Point(f.icon.iconanchor[0], f.icon.iconanchor[1]), infoWindowAnchor: new e.Size(f.icon.infowindowanchor[0], f.icon.infowindowanchor[1])}, b = new e.MarkerImage(i.image, i.iconSize, null, i.iconAnchor);
            ...
            var h = this;
            o.geocode({address: a.address}, function(d, g) {
            ...
            for (e = 0; e < b.markers.length; e += 1)
                c[e] = {latitude: b.markers[e].getPosition().lat(), longitude: b.markers[e].getPosition().lng()};
            a.markers = c;
            ...
            o.geocode({address: a}, function(a, d) {
            ...
            d.route({origin: a.from, destination: a.to, travelMode: l, unitSystem: m}, function(a, c) {
